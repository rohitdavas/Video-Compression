...
def prepare_unet_output(unet, unet_input, flows, warp):
    ...
    if warp:
        unet_output1, unet_output2 = warp_unet_outputs(flows, unet_output1, unet_output2)
...
def forward_model(model, cooked_batch, ctx_frames, args, v_compress,iterations, encoder_fuse_level, decoder_fuse_level):
    
    #-------
    #encoder, binarizer, decoder, unet = model
    encoder, binarizer,unet = model
    ...
    print("res_size ",res.size())
    #-----
    (encoder_h_1, encoder_h_2, encoder_h_3, decoder_h_1, decoder_h_2, decoder_h_3, decoder_h_4) = init_rnn(batch_size,height,width,args)
