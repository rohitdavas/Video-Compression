...
def get_codes_loader(args, output_suffix = ''):
    print("creating loader...")
    dset = NumpyDataset(args, output_suffix)

    loader = data.DataLoader(dataset = dset, 
                            batch_size = args.eval_batch_size,
                            num_workers = 2)
    print("loader for {} numpy codes ({} batches)".format(len(dset),
                                                         len(loader)))
    return loader
...
def default_loader(path):
    ...
    if count_load == 0:
        print("loaded image shape" ,cv2_img.shape)
        count_load +=1 
...
class NumpyDataset(data.Dataset):
    ...
    def __init__(self, args,output_suffix):
        path= os.path.join(args.out_dir, output_suffix, 'codes')
        #print("path of codes ", path)

        self.data_numpy_list = [x for x in glob.glob(path + '/' + '*.npz')]
        #print("files: ", self.data_numpy_list)

        self.data_list = []
        self.data_shape_list = []
        for ind in range(len(self.data_numpy_list)):
            data_slice_file_name = self.data_numpy_list[ind]
            #print("file: ",data_slice_file_name)

            data_i = np.load(data_slice_file_name)
            data_s = data_i['shape']
            data_i = np.unpackbits(data_i['codes'])
            self.data_list.append(data_i)
            self.data_shape_list.append(data_s)
        #print("len of codes array ", len(self.data_list))

    def __getitem__(self, index):

        self.data = np.asarray(self.data_list[index])
        self.data_shape = np.asarray(self.data_shape_list[index])
        return (torch.from_numpy(self.data).float(), torch.from_numpy(self.data_shape).float())
