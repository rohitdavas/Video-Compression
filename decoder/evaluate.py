...
def save_codes(name, codes,args, output_suffix):
  print("saving codes...")
  cur_eval_dir = os.path.join(args.out_dir, output_suffix, 'codes')
...
def run_eval(model, code_loader, args, output_suffix=''):
  ...
  for i, code in enumerate(code_loader):

      batch = Variable(code.cuda(), volatile=True)

      codes = eval_forward(model,code, args)
      
      save_codes(name= 'initial'+str(i), codes = codes,args= args, output_suffix = output_suffix)
